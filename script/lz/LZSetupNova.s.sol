...
contract LZSetupScript is Script {
    ...
    function run() public {
        ...
        // endpointV2.
        endpointV2.registerLibrary{gas: GAS_LIMIT}(address(simpleMessageLib));
        endpointV2.registerLibrary{gas: GAS_LIMIT}(address(sendUln302));
        endpointV2.registerLibrary{gas: GAS_LIMIT}(address(receiveUln302));
        ...
        dvn.setWorkerFeeLib{gas: GAS_LIMIT}(address(dvnFeeLib));
        dvn.setDefaultMultiplierBps{gas: GAS_LIMIT}(100); // TODO: verify 100 ?
        ...
        executorFeeLib = new ExecutorFeeLib(NATIVE_DECIMAL_RATE);
        {
            ...
            executor.setWorkerFeeLib{gas: GAS_LIMIT}(address(executorFeeLib));
        }
        ...
        sendUln302.setDefaultExecutorConfigs{gas: GAS_LIMIT}(executorConfigParams);
        sendUln302.setDefaultUlnConfigs(ulnConfigParams);

        // set receive uln config
        receiveUln302.setDefaultUlnConfigs(ulnConfigParams);

        endpointV2.setDefaultSendLibrary{gas: GAS_LIMIT}(REMOTE_EID, address(sendUln302));
        endpointV2.setDefaultReceiveLibrary{gas: GAS_LIMIT}(REMOTE_EID, address(receiveUln302), 0);
