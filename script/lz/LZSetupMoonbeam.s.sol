...
## For Moonbeam
`$ forge script ./script/lz/LZSetupMoonbeam.s.sol:LZSetupScript --private-key ${DEPLOYER_PRIVATE_KEY} --rpc-url ${MOONBEAM_TESTNET_RPC_URL} --broadcast  --verify --verifier blockscout --verifier-url $ETHSEPOLIA_VERIFIER_URL`

*/
/// @title LZ Setup Script
/// @author abhi3700
/// @notice LZ Setup Script
/// @dev Setup LZ for Nova & Moonbeam in 2 runs respectively
contract LZSetupScript is Script {
    ...
    // uint32 private constant REMOTE_EID = 1287; // for Moonbeam
    // string constant FILE_NAME = "./lzsetup_addresses_nova.txt";

    // Endpoint address, ids,
    // NOTE: Disable comment when deploying on Moonbeam
    address endpointV2Address = vm.envAddress("MOONBEAM_TESTNET_ENDPOINT_V2");
    uint32 private constant LOCAL_EID = 1287; // for Moonbeam
    uint32 private constant REMOTE_EID = 490_000; // for Nova
    string constant FILE_NAME = "./lzsetup_addresses_moonbeam.txt";
